...
composer.methods._renderMedia = function(oembed) {
	var self = this;
	var div = $("<div>");
	var card = new Echo.Conversations.NestedCard({
		"target": div,
		"context": this.config.get("context"),
		"width":  this.config.get("mediaWidth"),
		"data": oembed
	});
	var detachBtn = $(this.substitute({
		"template": '<div class="echo-primaryFont {class:close}">&times;</div>'
	})).one("click", function() {
		self.removeMedia(self._getDefinedMediaIndex(oembed));
	});
	div.append(detachBtn).appendTo(this.view.get("media"));
	return card;
};
